...
def _get_group_timeseries_from_systems(
    ...
    if len(group_data) > 0:
        ...
        ac_power_cols = [col for col in group_df.columns
                         if '_ac_power' in col
                         and 'clearsky' not in col]
        dc_power_cols = [col for col in group_df.columns
                         if '_dc_power' in col
                         and 'clearsky' not in col]
        ...
        ordered_columns = ["time", "ac_power", "clearsky_ac_power", "dc_power"] + all_cols
