...
syspath = Path(..., description="ID of system group to get", example=models.SYSTEM_ID)
...
async def update_system_group(
    ...
    name = system_group.name
    with storage.start_transaction() as st:
        out: models.StoredObjectID = st.update_system(system_id, name)
        response.headers["Location"] = request.url_for(
