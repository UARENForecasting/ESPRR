...
def test_user_foreign_key(cursor, group_id, user_id):
    cursor.execute(
        "select 1 from system_groups where id = uuid_to_bin(%s, 1)", group_id
    )
...
def test_mapping_group_foreign_key(cursor, group_id, user_id):
    ...
    assert cursor.fetchone()[0]
    cursor.execute("delete from system_groups where id = uuid_to_bin(%s, 1)", group_id)
    cursor.execute(
        "select 1 from system_group_mapping where group_id = uuid_to_bin(%s, 1)",
        group_id,
    )
...
def test_create_system_group(cursor, auth0_id, user_id):
    ...
    num = cursor.execute("call create_system_group(%s, %s)", (auth0_id, group_name))
    assert num == 1
...
def test_create_system_group_bad_user(cursor):
    with pytest.raises(IntegrityError) as err:
        cursor.execute("call create_system_group(%s, %s)", ("invalid", "another group"))
...
def test_add_system_to_group_no_system_access(cursor, auth0_id, group_id, system_def):
    num = cursor.execute(
        "call create_system(%s, %s, %s)",
        ("auth0|otheruser", "Another system", system_def[1]),
    )
    assert num == 1
    new_system_id = cursor.fetchone()[0]
    with pytest.raises(OperationalError) as err:
...
def test_update_system_group_name(cursor, auth0_id, group_id):
    ...
    cursor.execute(
        "call update_system_group(%s, %s, %s)", (auth0_id, group_id, new_name)
    )
    cursor.execute(
        "select name from system_groups where id = uuid_to_bin(%s, 1)", (group_id)
    )
...
def test_update_system_group_same_name(cursor, auth0_id, group_name, group_id):
    cursor.execute("select count(name) from systems where name = %s", group_name)
    assert len(cursor.fetchall()) == 1
    num = cursor.execute(
        "call create_system_group(%s, %s)", (auth0_id, "Another group")
    )
    assert num == 1
    new_system_id = cursor.fetchone()[0]
    with pytest.raises(IntegrityError) as err:
...
def test_remove_system_from_group(cursor, group_id, system_id, auth0_id):
    ...
    cursor.execute(
        "call remove_system_from_group(%s, %s, %s)", (auth0_id, system_id, group_id)
    )
    assert (
        cursor.execute(
            "select 1 from system_group_mapping where group_id = uuid_to_bin(%s, 1)",
...
def test_remove_system_from_group_bad_user(cursor, group_id, system_id, bad_user):
    with pytest.raises(OperationalError) as err:
        cursor.execute(
            "call remove_system_from_group(%s, %s, %s)", (bad_user, system_id, group_id)
        )
        assert err.value.args[0] == 1142


def test_remove_system_from_group_system_dne(cursor, group_id, auth0_id):
    with pytest.raises(OperationalError) as err:
        cursor.execute(
...
def test_delete_system_group(cursor, auth0_id, group_id):
    ...
    assert cursor.fetchone()[0]
    cursor.execute("call delete_system_group(%s, %s)", (auth0_id, group_id))
    cursor.execute(
        "select 1 from system_groups where id = uuid_to_bin(%s, 1)", group_id
    )
...
def test_delete_system_group_bad_user(cursor, bad_user, group_id):
    with pytest.raises(OperationalError) as err:
        cursor.execute("call delete_system_group(%s, %s)", (bad_user, group_id))
        assert err.value.args[0] == 1142


def test_delete_system_group_system_dne(cursor, auth0_id):
    with pytest.raises(OperationalError) as err:
        cursor.execute("call delete_system_group(%s, %s)", (auth0_id, str(uuid1())))
        assert err.value.args[0] == 1142
...
def test_get_group_systems_bad_user(cursor, bad_user, group_id):
    with pytest.raises(OperationalError) as err:
        cursor.execute("call get_group_systems(%s, %s)", (bad_user, group_id))
        assert err.value.args[0] == 1142


def test_get_group_systems_group_dne(cursor, auth0_id):
    with pytest.raises(OperationalError) as err:
        cursor.execute("call get_group_systems(%s, %s)", (auth0_id, str(uuid1())))
        assert err.value.args[0] == 1142
